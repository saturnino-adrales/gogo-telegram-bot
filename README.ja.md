...
### 1.0.14 — 1.0.10の修正を復活 + 再起動DMの抑制
- **リグレッション修正**: 1.0.11が誤って1.0.10の `handlerTimeout: Infinity`・`bot.catch`・オーナー通知ハンドラを巻き戻していた。復活。Telegrafの90秒ウォッチドッグは再び無効。
- スーパーバイザーが子プロセスに `GOGO_BOT_RESPAWN_COUNT` 環境変数を渡すように
- `Bot online` DMは初回起動時のみ送信 — サイレント再起動でオーナーに通知が飛ばなくなった
...
### 1.0.11 — 中間テキストをリアルタイム配信
- Claudeの思考過程の中間メッセージを、生成されたそばからTelegramに送信（従来はSDK完了まで溜め込んでまとめて送信していた）
- 最終結果が中間メッセージと重複している場合は送信をスキップ（重複表示を防止）
